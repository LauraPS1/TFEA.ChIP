...
The output of popular packages, such as DESeq2, for detection of differentially expressed genes from the analysis of count data from RNA-seq experiments produce tables with this information. To extract the information from a DESeqResults object the function *deseq2table* is available. The *hypoxia_DESeq* dataset is the output of a differential expression analysis performed on an RNAseq experiment analyzing the response to hypoxia of endothelial cells^[Tiana, M et al. The SIN3A histone deacetylase complex is required for a complete transcriptional response to hypoxia. https://doi.org/10.1101/182691 ] deposited at the NCBI's GEO repository (GSE89831).

```{r eval=TRUE,echo=TRUE,message=FALSE}
library(TFEA.ChIP)
data("hypoxia_DESeq",package="TFEA.ChIP") # load example of a DESeqResults object
hypoxia_table<-deseq2table(hypoxia_DESeq)
head(hypoxia_table)
```

After running *deseq2table*, your dataset will be ready to use with the rest of the package; gene names will be in Entrez Gene ID format and the resulting table is sorted by log2(Fold Change). For the rest of the vignette, we will use the dataset *hypoxia* as an example to explain how to perform the analysis if your dataset is originally in a data frame format.
...
Genes.Upreg <- hypoxia[hypoxia$log2FoldChange > 1 & hypoxia$padj <= 0.05,"Gene"] 
#extract vector with names of non-responsive genes
Genes.Control <- hypoxia[abs(hypoxia$log2FoldChange) < 0.25 & hypoxia$padj > 0.5,"Gene"]   
```
