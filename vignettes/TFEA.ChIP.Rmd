...
In case the user es interested in analyzing a particular set of genes that doesn't come from the input dataset, translating the IDs to Entrez Gene ID format is required. To that end its available the function *GeneID2entrez*:
```{r eval=TRUE,echo=TRUE,message=FALSE}
...
In this example, all 1122 datasets in the internal database were analyzed. However, we can restrict the analysis to a specific subset of the database and/or a given set of transcription factors. To this end we can produce and index of the tables of interest with the function *get_chip_index* and pass this index as an additional argument to *contingency_matrix*. 
...
```{r eval=TRUE,echo=TRUE}
chip_index<-get_chip_index(encodeFilter = TRUE) #selects ENCODE datasets only
